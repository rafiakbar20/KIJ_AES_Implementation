...
class AES:
    ...
    def decrypt(self, cipher):
        self.state = [[] for i in range(nb)]
        for r in range(4):
            for c in range(nb):
                self.state[r].append(cipher[r + 4 * c])
    ...
    def shift_rows(self, inv=False):
        ...
        if inv == False:  # encrypting
            for i in range(1, nb):
                self.state[i] = left_shift(self.state[i], count)
                count += 1
        else:  # decryptionting
            for i in range(1, nb):
                self.state[i] = right_shift(self.state[i], count)
                count += 1
    ...
    def add_round_key(self, round:int = 0):
        for col in range(nk):
            # nb*round is a shift which indicates start of a part of the KeySchedule
            s0 = self.state[0][col] ^ self.key_schedule[0][nb * round + col]
            s1 = self.state[1][col] ^ self.key_schedule[1][nb * round + col]
            s2 = self.state[2][col] ^ self.key_schedule[2][nb * round + col]
            s3 = self.state[3][col] ^ self.key_schedule[3][nb * round + col]
